...
pi-fusion は YAML config files を標準で読み込み、既存の JSON files も引き続きサポートします。

| Scope   | Path                      | 用途                                                   |
| ------- | ------------------------- | ------------------------------------------------------ |
| Global  | `~/.pi/agent/fusion.yml` | すべての repo で使いたい personal presets。             |
| Project | `.pi/fusion.yml`         | project と一緒に保持したい repo-specific presets。      |

project presets は同名の global presets を上書きします。pi-fusion は current working directory から上方向へ既存の `.pi/fusion.yml`、`.pi/fusion.yaml`、`.pi/fusion.json`、または `.git` directory を探すため、subdirectory から pi を起動しても repo-level config が見つかります。これは pi 自身の preset-extension pattern と同じ考え方です。global defaults が先、project-specific settings が後です。

pi-fusion は `~/.pi/agent/settings.json` には書き込みません。この file は pi の main settings と package list 用です。extension-specific data は別の `fusion.yml` に分けることで、削除や共有がしやすくなります。
...
| `/fusion preset save NAME`         | 現在の settings を `~/.pi/agent/fusion.yml` に保存します。   |
| `/fusion preset save-project NAME` | 現在の settings を `.pi/fusion.yml` に保存します。           |
...
config file に top-level の `presets` object があり、YAML または JSON syntax が正しいことを確認してください。不正な `fusion.yml`、`fusion.yaml`、`fusion.json` は extension を crash させずに無視されます。Project presets は `.pi/fusion.yml` に置きます。pi-fusion は pi が実行されている directory から上方向へ探索します。

### preset を保存したが、読み込んでも model が変わらない

model spec は pi が見つけられる `provider/model` 形式である必要があります。TUI model picker から current session model に戻すことも、`null` を使うか省略することもできます。

### preset を repo で共有したい

`/fusion preset save-project NAME` を使ってください。team で共有する場合は `.pi/fusion.yml` を commit します。personal な API cost の選択は `~/.pi/agent/fusion.yml` に置いてください。

### すべての fusion presets を削除したい

`~/.pi/agent/fusion.yml` と project の `.pi/fusion.yml` files を削除してください。extension は、再度 preset を保存したときだけ files を作り直します。以前 JSON を使っていた場合は、対応する `fusion.json` files も削除してください。

---

## Prompt Customization

`pi-fusion` が使うすべての prompts は完全にカスタマイズできます。初回実行時に、default prompts が global `fusion.yml` file に自動的に書き込まれます。そこで確認・編集でき、project ごとに上書きすることもできます。既存の `fusion.json` files も引き続きサポートされます。
...
| Scope   | Path                      | 用途                                                |
| ------- | ------------------------- | --------------------------------------------------- |
| Global  | `~/.pi/agent/fusion.yml` | すべての project で使う default templates。          |
| Project | `.pi/fusion.yml`         | team で共有する project-specific templates。         |

Project-level prompts は field ごとに global prompts を上書きします。pi-fusion は current working directory から上方向へ既存の `.pi/fusion.yml`、`.pi/fusion.yaml`、`.pi/fusion.json`、または `.git` directory を探すため、subdirectory から pi を起動しても repo-level config が見つかります。たとえば project file で `worker` だけを上書きし、global の `discovery`、`rewrite`、`actor` templates はそのまま使えます。

### YAML format

`fusion.yml` の top level に `prompts` section を追加します。
